...
cpquery(fit, event = (L == "yes"), evidence = (S == "yes"), n=100000)

#Inferência aproximada: mais exemplos
cpquery(fit, event = (L == "yes"), evidence = (S == "yes") | (S == "no"), n=100000)
cpquery(fit, event = (X == "yes"), evidence = ((T == "yes") & (S == "yes")), n=100000)
cpquery(fit, event = (X == "yes"), evidence = ((T == "no") & (S == "yes")), n=100000)

#Inferência aproximada: mais exemplos (2)
sim_sim = cpquery(fit, event = (L == "yes"), evidence = (S == "yes"), n=100000) 
sim_nao = cpquery(fit, event = (L == "yes"), evidence = (S == "no"), n=100000) 
nao_nao = cpquery(fit, event = (L == "no"), evidence = (S == "no"), n=100000) 
nao_sim = cpquery(fit, event = (L == "no"), evidence = (S == "yes"), n=100000) 

# isso mostra que quem fuma tem mais 10x mais chance de ter câncer de pulmão...
#
(sim_sim / nao_sim) / (sim_nao / nao_nao)

# o mesmo número você deve obter aqui
#
...
dagCru = pc.stable(dfDados, score = "bde")
plot(dagCru)

dagCru = hc(dfDados, score = "bic")
plot(dagCru)
